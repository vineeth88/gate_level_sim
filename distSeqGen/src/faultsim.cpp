...
class gateLevelCkt
{
	...
	int *numPropogated;	// number of times a fault is excited
	...
	std::vector<int> *exciteIndex;	// Vector index at which fault was excited
	std::vector<int> *propIndex;	// Vector index at which fault was propogated to next cycle
...
void faultSim(FILE *sigFile)
{
	...
	circuit->printGoodSig(sigFile, vecNum);
...
void gateLevelCkt::writeOutput(FILE *detFile, FILE *uflFile, FILE *eFile)
{
	...
	for (i=0; i<totalNumFaults; i++)
	{
		if (fstatus[i] == LOW_DETECT)	// detected
			fprintf(detFile, "%d %d %d %d;\n", i, fGate[i], fIO[i], fStuck[i]);
		else
		{
			if (fstatus[i] == POTENTIAL)	// potentially detected
			{
			...
			if (fstatus[i] == EXCITED_1_LEVEL) {	// excited but not detected
				fprintf(eFile, "%d %d %d %d; *** excited %d times [", i, fGate[i], fIO[i], fStuck[i], numExcited[i]);
				for (std::vector<int>::iterator it = exciteIndex[i].begin(); 
						it != exciteIndex[i].end(); ++it)
					fprintf(eFile, "%d ", *it);
				
				fprintf(eFile, "]\n propgated %d times [", numPropogated[i]);
				for (std::vector<int>::iterator it = propIndex[i].begin(); 
						it != propIndex[i].end(); ++it)
					fprintf(eFile, "%d ", *it);
				fprintf(eFile, "]\n");
		//		std::vector<int> exciteList;
		//		exciteList.push_back(exciteIndex[i][0]);
		//		int last_vector = exciteList.back();
//				int last_vector = exciteIndex[i][0];
//				fprintf(eFile, "%d", last_vector);
//				int index = 1;
//
//				std::vector<int>::iterator it = exciteIndex[i].begin();
//				++it;
//				for (; it != exciteIndex[i].end(); ++it) {
//					if (*it == last_vector + 1)
//						last_vector = *it;
//					else {
//						fprintf(eFile, "-%d, %d", last_vector, *it);
//						last_vector = *it;
//						index += 2;
//					}
//				}
//				if (index % 2)
//					fprintf(eFile, "-%d", exciteIndex[i].back());
//					
//				fprintf(eFile, "]\n");
			}
...
int gateLevelCkt::faultActive(int gateN, int ioN, int stuckN)
{
	...
	if (po[gateN])
	{
		...
		/* Comparing if value[gateN] = X i
			00 - stuckN, 11 - !stuckN, 01 - 0 */
			return(1);
		else
			return(0);
	}
	...
	while ((n < numF) && (fscaleIndex != -1))
	{
		...
		if ((stfCount[fscaleIndex] > 0) || faultActive(gateN, ioN, stuckN))
		{
			if (fstatus[fscaleIndex] == 0) {
				...
				exciteIndex[fscaleIndex].push_back(vecNum); // Vector_Index
				numExcited[fscaleIndex]++;
			}
			else if (fstatus[fscaleIndex] == EXCITED_1_LEVEL) {
				if (propIndex[fscaleIndex].size() && 
					(propIndex[fscaleIndex].back() == vecNum-1))
					propIndex[fscaleIndex].back() = vecNum;
				else {
					propIndex[fscaleIndex].push_back(vecNum);
					propIndex[fscaleIndex].push_back(vecNum);
				}
				numPropogated[fscaleIndex]++;
			}
						...
						for (i=0; i < numFaultsInserted; i++)
						{
							...
							{				// detect
								if (fstatus[fIndices[i]] < LOW_DETECT)
								{
									...
									if (OUTPUT_DET)
									{
										printf("%d %d %d; %d\n", fGate[fIndex], fIO[fIndex], fStuck[fIndex], fIndex);
									}
									if (!NO_FAULTDROP)
									{
										dropFault(fIndex);
...
void gateLevelCkt::setupFaults(char *cktName)
{
	...
	exciteIndex = new std::vector<int> [numFaults];
	propIndex = new std::vector<int> [numFaults];

	numExcited = new int[numFaults];
	numPropogated = new int[numFaults];
...
void gateLevelCkt::printSig(FILE *sigFile)
{
	...
	for (j=0; j<numFaultsInserted; j++)
	{
		...
		if (fstatus[fIndices[j]] == EXCITED_1_LEVEL) {
			...
			// now print the faulty output
			for (i=0; i<numff; i++) {
				if (currId[ff_list[i]] == faultId) {
					val1 = fvalue1[ff_list[i]] & mask;
					val2 = fvalue2[ff_list[i]] & mask;
				}
				else {
					val1 = value1[ff_list[i]] & mask;
					val2 = value2[ff_list[i]] & mask;
				}
...
//				 << ffStateStr << " : " 
//				 << fvStateStr << endl; 
			if (fvStateStr.compare(ffStateStr)) {
				fprintf(ffSigFile, "%s %d\n", fvStateStr.c_str(), fIndices[j]);
//				fprintf(ffSigFile, "%d: %s\n", fIndices[j], fvStateStr.c_str());
			}
		}
